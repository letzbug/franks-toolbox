...
function drawAnalyzer(canvas, arr, simple){
  ...
  function xForFreq(freq){
    ...
    return (Math.log10(clamped / minFreq) / Math.log10(maxFreq / minFreq)) * cw;
  }

  function binForFreq(freq){
    const maxBin = arr.length - 1;
    return Math.min(maxBin, Math.max(0, Math.round((freq / nyquist) * maxBin)));
  }

  function valueAtFreq(freq){
    const bin = binForFreq(freq);
    const v1 = arr[Math.max(0, bin - 1)] || 0;
    const v2 = arr[bin] || 0;
    const v3 = arr[Math.min(arr.length - 1, bin + 1)] || 0;
  ...
  ctx.strokeStyle = "rgba(65,111,171,.24)";
  ctx.lineWidth = 1;
  ...
  ctx.lineWidth = simple ? 2.4 : 3.2;
  ctx.beginPath();

  const points = simple ? 160 : 340;

  let detectedPeakFreq = minFreq;
  let detectedPeakValue = 0;

  for(let i=0;i<points;i++){
    const t = i / (points - 1);
    const freq = minFreq * Math.pow(maxFreq / minFreq, t);
    const v = valueAtFreq(freq);

    if(v > detectedPeakValue){
      detectedPeakValue = v;
      detectedPeakFreq = freq;
    }
  ...
  if(detectedPeakValue > 45){
    const peakX = xForFreq(detectedPeakFreq);
    const peakY = ch - 20 - (detectedPeakValue / 255) * (ch - 36);

    ctx.save();
    ctx.strokeStyle = "rgba(255,216,76,.92)";
    ctx.lineWidth = 2;
    ctx.shadowColor = "rgba(255,216,76,.55)";
    ctx.shadowBlur = 10;
    ctx.beginPath();
    ctx.moveTo(peakX,12);
    ctx.lineTo(peakX,ch - 22);
    ctx.stroke();
  ...
  ctx.fillStyle = "rgba(215,229,255,.82)";
  ...
  freqLabels.forEach(freq => {
    ...
    ctx.fillText(label, Math.min(cw - textWidth - 2, Math.max(2, x - textWidth / 2)), ch - 8);
  });
}
